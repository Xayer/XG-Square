...
public class Explosive : ToggleBehaviour
{
    ...
    // If true, the player dies of a nearby explosion
    public bool destroyPlayer = true;

    public bool avtivateByPlayerContact = true;
    ...
    public void Explode()
    {
        ...
        {
            // Destroy the player if destroyPlayer == true
            if(destroyPlayer)
                Character.Instance.Respawn();
            
            Character.MoveDirection = (Character.Instance.transform.position - transform.position).normalized*
                                      pushOnCharacter/
                                      Vector2.Distance(transform.position, Character.Instance.transform.position);
        }
